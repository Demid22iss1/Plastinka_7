...
app.get("/admin", requireAdmin, (req, res) => {
    ...
    db.get("SELECT COUNT(*) as count FROM products", [], (err, productCountResult) => {
        const productCount = productCountResult?.count || 0;
        db.get("SELECT COUNT(*) as count FROM players", [], (err, playerCountResult) => {
            const playerCount = playerCountResult?.count || 0;
            db.get("SELECT COUNT(*) as count FROM users", [], (err, userCountResult) => {
                const userCount = userCountResult?.count || 0;

                // Получаем список пластинок с количеством треков
                db.all(`
                    SELECT p.*, COUNT(t.id) as tracks_count 
                    FROM products p
                    LEFT JOIN tracks t ON t.product_id = p.id
                    GROUP BY p.id
                    ORDER BY p.id DESC
                `, [], (err, products) => {
                    if (err) products = [];

                    // Получаем список проигрывателей
                    db.all("SELECT * FROM players ORDER BY id DESC", [], (err, players) => {
                        if (err) players = [];

                        // Получаем список пользователей
                        db.all("SELECT id, username, role, avatar FROM users ORDER BY id DESC", [], (err, users) => {
                            if (err) users = [];

                            // Формируем HTML строки для таблицы пластинок
                            let productsRows = "";
                            for (const product of products) {
                                const imagePath = product.image ? `/uploads/${product.image}` : "/uploads/default.jpg";
                                const rating = product.avg_rating ? parseFloat(product.avg_rating).toFixed(1) : "0";
                                productsRows += `
                                    <tr>
                                        <td><img src="${imagePath}" class="table-img" onerror="this.src='/uploads/default.jpg'"></td>
                                        <td><strong>${escapeHtml(product.name)}</strong><br><span class="artist-sub">${escapeHtml(product.artist || '—')}</span></td>
                                        <td>${escapeHtml(product.genre || '—')}</td>
                                        <td>${escapeHtml(product.year || '—')}</td>
                                        <td>$${product.price}</td>
                                        <td class="stats-cell"><button onclick="viewRatings(${product.id}, 'product')"><i class="fas fa-star"></i> ${rating}</button></td>
                                        <td><span class="tracks-count"><i class="fas fa-music"></i> ${product.tracks_count || 0}</span></td>
                                        <td class="actions">
                            ...
                            // Формируем строки для проигрывателей
                            let playersRows = "";
                            for (const player of players) {
                                const imagePath = player.image ? `/photo/${player.image}` : "/photo/default-player.png";
                                playersRows += `
                                    <tr>
                                        <td><img src="${imagePath}" class="table-img" onerror="this.src='/photo/default-player.png'"></td>
                                        <td><strong>${escapeHtml(player.name)}</strong><br><span class="desc-sub">${escapeHtml((player.description || '').substring(0, 50))}${(player.description || '').length > 50 ? '…' : ''}</span></td>
                                        <td>$${player.price}</td>
                                        <td class="actions">
                            ...
                            // Формируем строки для пользователей
                            let usersRows = "";
                            for (const user of users) {
                                const avatarPath = user.avatar ? `/avatars/${user.avatar}` : "/avatars/default-avatar.png";
                                usersRows += `
                                    <tr>
                                        <td><img src="${avatarPath}" class="user-avatar-sm" onerror="this.src='/avatars/default-avatar.png'"></td>
                                        <td><strong>${escapeHtml(user.username)}</strong></td>
                                        <td><span class="role-badge ${user.role === 'admin' ? 'role-admin' : 'role-user'}">${user.role === 'admin' ? 'Админ' : 'Пользователь'}</span></td>
                                        <td class="stats-cell"><button class="reviews-btn" data-id="${user.id}" data-name="${escapeHtml(user.username)}"><i class="fas fa-comment-dots"></i> Отзывы</button></td>
                                        <td class="stats-cell"><button class="favorites-btn" data-id="${user.id}"><i class="fas fa-heart"></i> Избранное</button></td>
                                        <td class="stats-cell"><button class="cart-btn" data-id="${user.id}"><i class="fas fa-shopping-cart"></i> Корзина</button></td>
                                        <td class="actions">
                                            <button class="edit-user" data-id="${user.id}" data-name="${escapeHtml(user.username)}" data-role="${user.role}"><i class="fas fa-edit"></i></button>
                                            ${user.username !== 'admin' ? `<button class="delete-user" data-id="${user.id}"><i class="fas fa-trash"></i></button>` : ''}
                                        </td>
                                    </tr>
                                `;
                            }

                            // Отправляем готовую админ-панель
                            res.send(adminPanelHTML(productCount, playerCount, userCount, productsRows, playersRows, usersRows, username));
                        });
                    });
                });
            });
...
// ============================================================
// ===================== API ДЛЯ ТРЕКОВ В АДМИНКЕ =============
// ============================================================
app.get("/admin/get-tracks/:productId", requireAdmin, (req, res) => {
    const productId = req.params.productId;
    db.all("SELECT id, track_number, name, duration FROM tracks WHERE product_id = ? ORDER BY track_number ASC", [productId], (err, tracks) => {
        if (err) return res.json([]);
        res.json(tracks.map(t => ({ name: t.name, duration: t.duration })));
    });
});

// Обновлённый обработчик сохранения товара с поддержкой треков
app.post("/admin/save-item", requireAdmin, upload.fields([{ name: 'image' }, { name: 'audio' }]), (req, res) => {
    const { type, id, name, artist, genre, year, price, description, tracks } = req.body;
    const imageFile = req.files?.image?.[0];
    const audioFile = req.files?.audio?.[0];
    let tracksData = [];
    try {
        tracksData = tracks ? JSON.parse(tracks) : [];
    } catch(e) { tracksData = []; }

    if (type === 'product') {
        const saveProduct = (productId) => {
            // Сохраняем треки: удаляем старые, вставляем новые
            db.run("DELETE FROM tracks WHERE product_id = ?", [productId], (err) => {
                if (err) console.error("Ошибка удаления старых треков:", err);
                const stmt = db.prepare("INSERT INTO tracks (product_id, track_number, name, duration) VALUES (?, ?, ?, ?)");
                tracksData.forEach((track, idx) => {
                    if (track.name && track.name.trim()) {
                        stmt.run(productId, idx + 1, track.name.trim(), track.duration || '');
                    }
                });
                stmt.finalize();
                res.json({ success: true });
        ...
        if (id && id !== '' && id !== 'undefined') {
            ...
            db.run("INSERT INTO products (name, artist, price, image, audio, description, genre, year) VALUES (?,?,?,?,?,?,?,?)",
                ...
                function(err) {
                    ...
                    saveProduct(this.lastID);
                });
        }
    } else {
        // Для проигрывателей треки не нужны
        if (id && id !== '' && id !== 'undefined') {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav-link:hover {
            background: rgba(215, 67, 7, 0.1);
            color: var(--primary);
        }
        
        .nav-link.active {
            background: rgba(215, 67, 7, 0.15);
            color: var(--primary);
        ...
        .btn-primary, .btn-secondary, .btn-settings, .btn-user-add {
            padding: 10px 20px;
            border-radius: 8px;
            border: none;
            cursor: pointer;
        ...
        .btn-primary:hover, .btn-secondary:hover, .btn-settings:hover {
            opacity: 0.9;
            transform: translateY(-1px);
        }
        ...
        .edit-product, .delete-product, .edit-player, .delete-player, .edit-user, .delete-user {
            width: 30px;
            height: 30px;
            border-radius: 6px;
            border: none;
        ...
        .edit-product, .edit-player, .edit-user {
            background: #2a2a2a;
            color: #ffc107;
        }
        
        .edit-product:hover, .edit-player:hover, .edit-user:hover {
            background: #ffc107;
            color: #000;
        }
        
        .delete-product, .delete-player, .delete-user {
            background: #2a2a2a;
            color: #f44336;
        }
        
        .delete-product:hover, .delete-player:hover, .delete-user:hover {
            background: #f44336;
            color: white;
        }
...
<div class="admin-app">
    ...
    <aside class="sidebar" id="sidebar">
        <div class="sidebar-header">
            <h2><i class="fas fa-crown"></i> Plastinka</h2>
            <p>Админ панель</p>
        </div>
        
        <ul class="nav-menu">
            <li class="nav-item">
                <a class="nav-link active" data-tab="products">
                    <i class="fas fa-record-vinyl"></i>
                    <span>Пластинки</span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" data-tab="players">
                    <i class="fas fa-headphones"></i>
        ...
        <div class="user-info-sidebar">
            ...
            <div class="user-role-sidebar"><i class="fas fa-shield-alt"></i> Администратор</div>
    ...
    <main class="main-content">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-label">📀 Пластинок</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">${playerCount}</div>
                <div class="stat-label">🎵 Проигрывателей</div>
            </div>
            <div class="stat-card">
            ...
            <div class="stat-card">
                <div class="stat-value">${productCount + playerCount}</div>
                <div class="stat-label">📦 Всего товаров</div>
            </div>
        </div>
        
        <!-- Кнопки действий (динамически меняются) -->
        <div class="action-buttons" id="actionButtons">
            <button class="btn-primary" id="addProductBtn"><i class="fas fa-plus-circle"></i> Добавить пластинку</button>
            <button class="btn-secondary" id="addPlayerBtn" style="display: none;"><i class="fas fa-plus-circle"></i> Добавить проигрыватель</button>
            <button class="btn-user-add" id="addUserBtn" style="display: none;"><i class="fas fa-user-plus"></i> Добавить пользователя</button>
        ...
        <div id="products-tab" class="tab-content active">
            <div class="table-container">
                <table>
                    <thead>
                        <tr><th>Обложка</th><th>Название / Исполнитель</th><th>Жанр</th><th>Год</th><th>Цена</th><th>Рейтинг</th><th>Треки</th><th>Действия</th></tr>
                    </thead>
                    <tbody>${productsRows || '<tr><td colspan="8"><div class="empty-data">Нет пластинок</div></td></tr>'}</tbody>
                </table>
            </div>
        </div>
        
        <!-- Вкладка Проигрыватели -->
        <div id="players-tab" class="tab-content">
            <div class="table-container">
                <table>
                    <thead>
                        <tr><th>Изображение</th><th>Название / Описание</th><th>Цена</th><th>Действия</th></tr>
                    </thead>
                    <tbody>${playersRows || '<tr><td colspan="4"><div class="empty-data">Нет проигрывателей</div></td></tr>'}</tbody>
                </table>
            </div>
        </div>
        ...
        <div id="users-tab" class="tab-content">
            <div class="table-container">
                <table>
                    <thead>
                        <tr><th>Аватар</th><th>Имя</th><th>Роль</th><th>Отзывы</th><th>Избранное</th><th>Корзина</th><th>Действия</th></tr>
                    </thead>
                    <tbody>${usersRows || '<tr><td colspan="7"><div class="empty-data">Нет пользователей</div></td></tr>'}</tbody>
                </table>
            </div>
        </div>
    </main>
...
<!-- Модальное окно для товаров (с треками) -->
<div id="itemModal" class="modal-overlay">
    <div class="modal-content">
        <button type="button" class="modal-close" onclick="closeModal('itemModal')">&times;</button>
        <h3 id="modalTitle">Добавить товар</h3>
        <form id="itemForm" enctype="multipart/form-data">
            <input type="hidden" id="itemId" name="id">
            <input type="hidden" id="itemType" name="type">
            <input type="text" id="itemName" name="name" placeholder="Название" required>
            <input type="text" id="itemArtist" name="artist" placeholder="Исполнитель">
            <input type="text" id="itemGenre" name="genre" placeholder="Жанр">
            <input type="text" id="itemYear" name="year" placeholder="Год">
            <input type="number" id="itemPrice" name="price" placeholder="Цена" step="0.01" required>
            <textarea id="itemDescription" name="description" placeholder="Описание"></textarea>
            <input type="file" id="itemImage" name="image" accept="image/*">
            <input type="file" id="itemAudio" name="audio" accept="audio/*">
            
            <div id="tracksSection" class="tracks-section" style="display: none;">
                <h4><i class="fas fa-music"></i> Список треков (необязательно)</h4>
                <div id="tracksList" class="tracks-list"></div>
                <button type="button" class="add-track-btn" onclick="addTrackField()">
            ...
            <div class="modal-buttons">
                ...
                <button type="button" onclick="closeModal('itemModal')">Отмена</button>
            </div>
        </form>
    </div>
</div>
...
<!-- Модальные окна для просмотра данных пользователя -->
...
<script>
    ...
    function switchTab(tabId) {
        ...
        const titles = {
            products: { main: 'Пластинки', sub: 'Управление коллекцией пластинок' },
            players: { main: 'Проигрыватели', sub: 'Управление проигрывателями' },
            users: { main: 'Пользователи', sub: 'Управление пользователями' }
        };
        
        document.querySelector('.page-title h1').innerText = titles[tabId].main;
        document.querySelector('.page-title p').innerText = titles[tabId].sub;
    ...
    function renderTracksList() {
        ...
        if (currentTracks.length === 0) {
            container.innerHTML = '<div class="empty-data" style="padding: 20px;">Нет добавленных треков</div>';
            return;
        }
        ...
        for (let i = 0; i < currentTracks.length; i++) {
            ...
            html += '<div class="track-item" data-index="' + i + '">' +
                '<input type="text" class="track-name" placeholder="Название трека" value="' + escapeHtml(track.name || '') + '">' +
                '<input type="text" class="track-time" placeholder="Длительность (например, 3:45)" value="' + escapeHtml(track.duration || '') + '">' +
                '<button type="button" class="remove-track" onclick="removeTrack(' + i + ')"><i class="fas fa-trash"></i></button>' +
                '</div>';
        }
        container.innerHTML = html;
        
        for (let i = 0; i < currentTracks.length; i++) {
            const trackDiv = container.children[i];
            if (trackDiv) {
                const nameInput = trackDiv.querySelector('.track-name');
                const timeInput = trackDiv.querySelector('.track-time');
                if (nameInput) {
                    nameInput.onchange = (function(idx) { return function() { currentTracks[idx].name = this.value; }; })(i);
                }
                if (timeInput) {
                    timeInput.onchange = (function(idx) { return function() { currentTracks[idx].duration = this.value; }; })(i);
                }
            }
        }
    }
    ...
    function addTrackField() {
        currentTracks.push({ name: '', duration: '' });
        renderTracksList();
    }
    ...
    function loadTracks(productId) {
        fetch('/admin/get-tracks/' + productId)
            .then(res => res.json())
            .then(data => {
                currentTracks = data || [];
                renderTracksList();
            })
            .catch(() => { currentTracks = []; renderTracksList(); });
    }
    ...
    function openAddUserModal() {
        document.getElementById('userModalTitle').innerText = 'Добавить пользователя';
    ...
    function editUser(id, name, role) {
        document.getElementById('userModalTitle').innerText = 'Редактировать пользователя';
        document.getElementById('editUserId').value = id;
        document.getElementById('editUsername').value = name;
        document.getElementById('editPassword').value = '';
        document.getElementById('editPassword').placeholder = 'Новый пароль (оставьте пустым, чтобы не менять)';
    ...
    document.querySelectorAll('.delete-product').forEach(btn => {
        btn.onclick = function() { deleteItem('product', this.dataset.id); };
    });
    
    document.querySelectorAll('.delete-player').forEach(btn => {
        btn.onclick = function() { deleteItem('player', this.dataset.id); };
    });
    
    document.querySelectorAll('.edit-user').forEach(btn => {
        btn.onclick = function() { editUser(this.dataset.id, this.dataset.name, this.dataset.role); };
    });
    
    document.querySelectorAll('.delete-user').forEach(btn => {
        btn.onclick = function() { deleteUser(this.dataset.id); };
    });
    
    // Отправка ответа на отзыв
    window.sendReply = function(reviewId, productId, userId) {
        ...
        fetch('/admin/send-review-reply', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reviewId: reviewId, productId: productId, userId: userId, reply: replyText }) })
            ...
            .then(data => {
                if (data.success) {
                    const replyDiv = document.getElementById('reply-div-' + reviewId);
    ...
    document.querySelectorAll('.reviews-btn').forEach(btn => {
        btn.onclick = function() {
            const userId = this.dataset.id, userName = this.dataset.name;
            ...
            fetch('/admin/user-reviews/' + userId).then(res => res.json()).then(data => {
                ...
                for (let r of data) {
                    ...
                    html += '<div class="user-data-item"><div class="user-data-header"><img src="/uploads/' + escapeHtml(r.product_image) + '" class="user-data-image"><div class="user-data-info"><div class="user-data-name">' + escapeHtml(r.product_name) + '</div><div class="user-data-artist">' + escapeHtml(r.product_artist) + '</div><div class="user-data-rating">' + stars + ' (' + r.rating + '/5)</div><div class="user-data-date">' + new Date(r.created_at).toLocaleDateString() + '</div></div></div>' + (r.comment ? '<div class="user-data-comment">"' + escapeHtml(r.comment) + '"</div>' : '') + '<div id="reply-div-' + r.id + '">' + (r.admin_reply ? '<div class="admin-reply"><div class="admin-reply-text">Администратор ответил:</div><div class="admin-reply-content">' + escapeHtml(r.admin_reply) + '</div></div>' : '') + '</div><div class="reply-form"><input type="text" id="reply-input-' + r.id + '" placeholder="Ответ..."><button onclick="sendReply(' + r.id + ', ' + r.product_id + ', ' + userId + ')">Ответить</button></div></div>';
                }
    ...
    document.querySelectorAll('.cart-btn').forEach(btn => {
        btn.onclick = function() {
            ...
            fetch('/admin/user-cart/' + userId).then(res => res.json()).then(data => {
                ...
                document.getElementById('cartTotal').innerHTML = '<span style="color: var(--primary); text-align: right; display: block;">Итого: $' + total + '</span>';
            });
        };
    });
    ...
    document.getElementById('itemForm').onsubmit = function(e) {
        ...
        formData.append('tracks', JSON.stringify(currentTracks));
        fetch('/admin/save-item', { method: 'POST', body: formData })
            .then(res => res.json())
    ...
    document.getElementById('userForm').onsubmit = function(e) {
        ...
        fetch('/admin/update-user', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
            .then(res => res.json())
            .then(data => { if (data.success) location.reload(); else alert('Ошибка'); });
    };
